...
let DATABASE = rethinkdb.db || 'CS3249';
let TABLES = ['users', 'community', 'household', 'patient', 'sample'];

r.connect(rethinkdb).then(conn => {
  console.log(' [-] Database Setup');
  return createDbIfNotExists(conn)
    .then(() =>
      Promise.all(TABLES.map(table => deleteTableIfExists(conn, table))))
    .then(() =>
...
function deleteTableIfExists(conn, table) {
  return getTableList(conn).then(list => {
    if (list.indexOf(table) !== -1) {
      return deleteTable(conn, table);
    } else {
      console.log(' [!] Table already deleted:', table);
      return Promise.resolve(true);
    }
  });
}
...
function deleteTable(conn, table) {
  console.log(' [-] Delete Table:', table);
  return r.db(DATABASE).tableDrop(table).run(conn);
}
